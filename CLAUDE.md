...
- **Reaching the backend:** use the helpers in `src/frontend/backendApi.ts` (`apiUrl('/health')`, `getApiBaseUrl()`, `getBackendOrigin()`). They target the backend on port **5174** at the current hostname. Don't hardcode API URLs.
...
2. **Long-term storage** — anything needing persistence (e.g. `todos.json`, `sprite-groups.json`).
...
- Routes live in `src/backend/routes/` and are mounted under `/api` in `index.ts`: `todos`, `mbta`, `llm-duo-chat`, `wikipedia-story`, `sprite-groups`. Health: `GET /api/health`.
- Adding a route: create `routes/<name>.ts` exporting a router, import and `app.use('/api', <name>Router)` in `index.ts`. If it needs init (storage, WebSocket), export an init fn and call it before/after `server.listen` like `initTodoStorage()` / `initLLMDuoChatWebSocket(server)`.
...
- Run with `tsx watch` in dev; built with `tsc -p src/backend/tsconfig.json`. Note ESM `.js` import specifiers in TS source (e.g. `from './routes/todos.js'`).
