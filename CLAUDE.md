...
- Routes live in `src/backend/routes/` and are mounted under `/api` in `index.ts`: `llm-duo-chat`, `sky`, `sprite-groups`. Health: `GET /api/health`. Migrated slices (e.g. `todo`, `transit`, `wikistory`, `image-hunt`) mount instead at `/api/<id>` via `mount(id, slice)` in `src/server.ts` — see `src/experiments/<id>/server/index.ts`.
- Adding a legacy route: create `routes/<name>.ts` exporting a router, import and `app.use('/api', <name>Router)` in `index.ts`. If it needs init (storage, WebSocket), export an init fn and call it before/after `server.listen` like `initLLMDuoChatWebSocket(server)`. A migrated **slice** instead declares its own `init?: () => Promise<void>` directly on the `SliceServer` object it exports (see `src/platform/server/slice.ts`) — the host's `mount(id, slice)` calls it automatically, so there's no separate init fn to wire up in `src/server.ts`. Most slices need none at all: `createJsonStore` already returns its fallback when the backing file is missing, which is why `image-hunt`'s old `initSessionStorage` disappeared entirely on migration.
